<template>
	<div class="modal-body d-flex flex-gap" :class="{ 'cursor-wait': !valid }">
		<div class="d-flex flex-column w-25 flex-gap px-2 overflow-auto" style="max-height: 450px;">
			<button class="btn btn-exam btn-warning w-100" v-for="(exam, index) in allExams" :key="index"
...
export default {
	...
	created() {
		this.examenes = [];
		this.dataExamBasic.forEach(basico =>{
			let contenido = JSON.parse(basico.exam)
			this.examenes.push({
