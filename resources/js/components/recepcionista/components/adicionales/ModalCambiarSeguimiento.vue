    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <select class="form-select" v-model="seguir.idSeguimiento">
					<option v-for="seguimiento in seguimientos" v-if="seguimiento.id<=8 || seguimiento.id==13 || seguimiento.id== 14 || seguimiento.id== 15 || seguimiento.id== 16 " :value="seguimiento.id">{{ seguimiento.icono }} {{seguimiento.seguimiento}}</option>
				</select>
				...
				<ol class="list-group list-group-numbered">
					<li class="list-group-item list-group-item-action d-flex justify-content-between align-items-start" v-for="historial in historiales">
						<div class="ms-2 me-auto">
							<div class="fw-bold">{{ historial.icono }} {{historial.seguimiento}}</div>
							<p class="mb-0 text-capitalize"><i class="far fa-comment"></i> {{ historial.observaciones ? historial.observaciones : 'Sin respuesta' }}</p>
						</div>
						<span class="badge rounded-pill text-dark"><i class="far fa-calendar-alt"></i> {{ fechaLatam(historial.registro) }}</span>
					</li>
				</ol>
		...
		asginarSeguimiento(){
			...
			.then(response => { //console.log(response.data);
				...
				if( parseInt(response.data) > 0)
					...
					this.limpiarCampos();
					alertify.notify('<i class="fa-regular fa-calendar-check"></i> Datos registrados ', 'success', 5)
			})
		},
		limpiarCampos(){
