		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content modal-sm">
					...
					<form action="">
						<div class="form-group row">
							<div class="col-sm-12">
								...
								<p class="lead text-capitalize mb-2 fw-bold"> <span>{{ (dataCita.patient?.name ?? '').toLowerCase() }}
										{{ (dataCita.patient?.nombres ?? '').toLowerCase() }}</span></p>
								<p class="lead mb-0"><small>Precio a cobrar: S/</small> {{ parseFloat(dataCita.payment.price).toFixed(2)
									}}</p>
								<p v-if="dataCita.payment.rebaja > 0" class="lead mb-0"><small>Rebajado: S/</small> {{
									parseFloat(dataCita.payment.rebaja).toFixed(2) }}</p>
								<p v-if="dataCita.payment.adelanto > 0" class="lead mb-0"><small>Adelanto: </small> S/ {{
									parseFloat(dataCita.payment.adelanto).toFixed(2) }}</p>
								<p v-if="dataCita.payment.razonAdelanto" class="mb-0"><small>Obs. o Fecha: {{
									dataCita.payment.razonAdelanto }}</small></p>
							</div>
							<div v-if="dataCita.byDoctor == 1">
								<label for="">Aplicar rebaja (S/)
									<br><small>Máximo: S/ {{ (maximo).toFixed(2) }}</small>
								</label>
								<input type="number" class="form-control" min="0" :max="maximo" id="txtrebajaDoctor"
									v-model="caso.rebaja" @keyup="agregarrebaja()">
							</div>

							<div class="col-sm-12 mt-2">
								<label class="mt-2 mb-0" for="">Estado de pago</label>
								<select v-if="caso.pago == 1 || caso.pago == 3" class="form-select" name="pay_status" id="pay_status"
									v-model="caso.pago" @change="caso.monto_adelanto = 0">
						...
						<div class="form-group">
							...
							<textarea class="form-control" name="observation" id="observation" cols="10" rows="2"
								v-model="dataCita.payment.observation"></textarea>
				...
				<div class="modal-footer border-0">
					...
					<button v-if="dataCita.payment.pay_status == 1" @click="update()" type="button"
						class="btn btn-outline-primary"><i class="fas fa-save"></i> Guardar pago</button>
					<a target="_blank" :href="`/api/pdfCupon/${dataCita.id}?token=${token}`"
						v-if="dataCita.payment.pay_status != 1" class="btn btn-outline-success">Cupón</a>
				</div>
			</div>
		</div>
	</div>
